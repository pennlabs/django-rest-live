...
The goal of this project is to be as close as possible to a drop-in realtime/websockets solution for projects already
...
endpoints generated by ViewSets and views to get initial data to populate a page, as well as any write-driven
...
model class. To do so, `serializer_class` needs to be defined on the `ViewSet`. You'll receive a runtime error if
the model class cannot be inferred. If you rely on `get_serializer_class()` in your `ViewSet` for dynamically
setting the serializer, then you should explicitly define the `model_class` on your `ViewSet`:
